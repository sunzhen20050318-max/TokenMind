...
from sun_agent import __logo__, __version__
from sun_agent.config.paths import get_workspace_path
...
def _init_prompt_session() -> None:
    ...
    from sun_agent.config.paths import get_cli_history_path
...
def onboard(
    ...
    if wizard:
        from sun_agent.cli.onboard_wizard import run_onboard
...
def _make_provider(config: Config):
    ...
    from sun_agent.providers.base import GenerationSettings
    from sun_agent.providers.openai_codex_provider import OpenAICodexProvider
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from sun_agent.config.loader import load_config, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    import json
    from sun_agent.config.loader import get_config_path
...
def gateway(
    ...
    from sun_agent.config.loader import get_config_path
    from sun_agent.config.paths import get_cron_dir
    from sun_agent.cron.service import CronService
    from sun_agent.cron.types import CronJob
    from sun_agent.heartbeat.service import HeartbeatService
...
def web(
    ...
    from sun_agent.heartbeat.service import HeartbeatService
    from sun_agent.session.manager import SessionManager
    from sun_agent.server.app import create_app
    ...
    # Set cron callback on MessageTool
    async def on_progress(content: str, *, tool_hint: bool = False) -> None:
        pass  # Web UI handles progress via WebSocket
    ...
    async def run():
        ...
        try:
            ...
            if outbound_dispatcher_task:
                ...
                try:
                    await outbound_dispatcher_task
                except asyncio.CancelledError:
                    pass
            await agent.close_mcp()
            cron.stop()
...
def agent(
    ...
    if logs:
        logger.enable("sun_agent")
    else:
        logger.disable("sun_agent")
...
def _get_bridge_dir() -> Path:
    ...
    from sun_agent.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # sun_agent/bridge (installed)
...
def channels_login():
    ...
    from sun_agent.config.loader import load_config
    from sun_agent.config.paths import get_runtime_subdir
...
def status():
    ...
    if config_path.exists():
        from sun_agent.providers.registry import PROVIDERS
