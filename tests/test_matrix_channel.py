...
import tokenmind.channels.matrix as matrix_module
from tokenmind.bus.events import OutboundMessage
from tokenmind.bus.queue import MessageBus
from tokenmind.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from tokenmind.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("tokenmind.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "tokenmind.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("tokenmind.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "tokenmind.channels.matrix.asyncio.create_task", _fake_create_task
    )
