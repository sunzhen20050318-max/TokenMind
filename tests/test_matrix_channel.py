...
import sun_agent.channels.matrix as matrix_module
from sun_agent.bus.events import OutboundMessage
from sun_agent.bus.queue import MessageBus
from sun_agent.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from sun_agent.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("sun_agent.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "sun_agent.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("sun_agent.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "sun_agent.channels.matrix.asyncio.create_task", _fake_create_task
    )
