...
from tokenmind.cli import commands
...
def mock_prompt_session():
    ...
    with patch("tokenmind.cli.commands._PROMPT_SESSION", mock_session), \
         patch("tokenmind.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("tokenmind.cli.commands.PromptSession") as MockSession, \
         patch("tokenmind.cli.commands.FileHistory") as MockHistory, \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch.object(commands.console, "status", return_value=spinner), \
         patch("tokenmind.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = commands._ThinkingSpinner(enabled=True)
        with thinking:
