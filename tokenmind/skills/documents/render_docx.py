...
def _resolve_soffice() -> str:
    """Locate soffice across macOS / Linux / Windows. Raises if missing.

    Lookup order: ``soffice``/``soffice.exe`` on PATH, then well-known install
    paths (macOS DMG, Windows Program Files, Linux /opt, snap).
    """
...
def _profile_uri(profile_dir: str) -> str:
    """Cross-platform ``file://`` URI for a directory path."""
...
def convert_to_pdf(
    ...
    profile_url = _profile_uri(user_profile)
