"""Interactive onboarding questionnaire for tokenmind."""
...
from tokenmind.cli.model_info import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]tokenmind[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from tokenmind.config.schema to get
    ...
    # Check if current value is the default (262144)
    # We only auto-fill if the user hasn't changed it from default
    from tokenmind.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"tokenmind.channels.{name}")
