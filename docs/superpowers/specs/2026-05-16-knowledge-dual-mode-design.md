...
- **Wiki KB 删除时**：active 在该 KB 的会话怎么办？建议：删除时把所有激活到该 KB 的会话 `active_wiki_kb_id` 设为 null。需确认。
- **Wiki KB 重命名 / purpose 变更**：要不要触发 system prompt 缓存失效？现在 system prompt 每轮都重建，应该不需要。
- **会话历史中调过的 wiki 工具结果**：切换 KB 后这些 tool_result 仍留在历史里。LLM 看到时会困惑吗？需要在切换时塞一条系统消息说明？
- **purpose.md 谁来填**：创建 Wiki KB 时用户必须填？还是 LLM 根据 KB 名字自动生成草稿？建议第一版只让用户填短描述（即 KB description），purpose.md 用模板自动生成。
